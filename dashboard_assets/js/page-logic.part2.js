  ...
  try {
    ...
    if (!subscriptions) {
      ...
      if (window.dbManager) {
        ...
        let result;
        if (userData.isAdmin) {
          result = await window.dbManager.getAllSubscriptionsForAdmin();
        } else {
      ...
      filteredSubscriptions.forEach(sub => {
        ...
        card.innerHTML = `
          <div class="card-header">
            <div class="card-title">${sub.customer_name || sub.customerName || 'غير محدد'}</div>
            <div class="card-subtitle">${sub.email}</div>
          </div>
          <div class="card-body">
            ...
            <div class="card-field">
              <div class="card-field-label">نوع الاشتراك</div>
              <div class="card-field-value">${getSubscriptionTypeName(sub.subscription_type || sub.type)}</div>
            </div>
            <div class="card-field">
              <div class="card-field-label">العملة</div>
              <div class="card-field-value">${getCurrencyName(sub.currency)}</div>
            </div>
            <div class="card-field">
            ...
            <div class="card-field">
              ...
              <div class="card-field-value">
                <span class="px-2 py-1 rounded text-sm ${getStatusClass(isExpired, isExpiring)}">
                  ${getStatusText(isExpired, isExpiring)}
                </span>
              </div>
            </div>
            <div class="card-field">
              ...
              <div class="card-field-value">
                ${sub.notes ? `<i class="fas fa-sticky-note text-yellow-500 ml-1"></i>${sub.notes}` : '<span class="text-gray-500">-</span>'}
              </div>
            </div>
          </div>
          <div class="card-actions">
            ${updateTableActionButtons(sub, 'regular')}
          </div>
        `;
